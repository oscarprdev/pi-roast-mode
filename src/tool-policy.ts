...
export const SAFE_BUILTIN_ROAST_TOOLS = new Set(["read", "bash", "grep", "find", "ls"]);
export type RoastModeToolPolicy = "read-only" | "limited" | "user-opt-in" | "blocked";
...
export function classifyRoastModeTool(tool: ToolInfo): RoastModeToolPolicy {
	...
	return SAFE_BUILTIN_ROAST_TOOLS.has(tool.name) ? "read-only" : "blocked";
}

export function canSelectToolInRoastMode(tool: ToolInfo) {
	return classifyRoastModeTool(tool) !== "blocked";
}
