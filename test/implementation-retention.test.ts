...
import type { ImplementationRoastRetention } from "../src/settings.js";
import { restoreRoastModeState } from "../src/state.js";
import { createMockContext, createMockPi } from "./support.js";

const ROAST = "# Retained roast\n\n1. Keep the complete handoff.";
...
async function beginImplementation(
	...
	roastMode(mock.pi, {
		readSettings: async () => ({
			...
			settings: {
				...
				implementationRoastRetention: retention,
			},
		}),
	});
	...
	const complete = mock.tools.find((tool) => tool.name === "roast_mode_complete")?.execute as
...
test("active implementation state captures retention and legacy state restores as keep", () => {
	const legacy = restoreRoastModeState(
		[
			stateEntry({
				enabled: false,
	...
	const explicit = restoreRoastModeState(
		[
			stateEntry({
				enabled: false,
				awaitingAction: false,
...
test("keep leaves the accepted roast active after context and settlement", async () => {
	...
	assert.match(JSON.stringify(transformed.messages), /Retained roast/);
...
test("handoff-only waits for the exact queued handoff, preserves its first context, then clears", async () => {
	...
	assert.match(JSON.stringify(firstImplementationContext.messages), /Retained roast/);
	...
	assert.doesNotMatch(JSON.stringify(later.messages), /Retained roast/);
});
...
test("Implement menus preview each configured retention outcome before confirmation", async () => {
	for (const [retention, preview] of [
		["keep", "Keep roast active until /roast exit"],
		["clear-on-start", "Use the roast for the implementation handoff only"],
...
test("changing Settings does not retroactively change an active implementation", async () => {
	const directory = await mkdtemp(join(tmpdir(), "pi-roast-retention-settings-"));
	...
	try {
		...
		roastMode(mock.pi, {
			...
			readSettings: async () => ({
				...
				settings: {
					thinkingLevel: "inherit" as const,
					implementationRoastRetention: "keep" as const,
				},
			}),
		});
		...
		assert.equal(
			(
				JSON.parse(await readFile(settingsPath, "utf8")) as {
					implementationRoastRetention?: string;
				}
