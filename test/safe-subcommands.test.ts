...
test("active Roast mode enforces session-loaded safe subcommands", async () => {
	await withAgentDir(async (agentDir) => {
		...
		assert.equal(
			...
			undefined,
			"inactive Roast mode must not enforce its shell policy",
		);
		...
		assert.deepEqual(
			await hook({ toolName: "bash", input: { command: compoundCommand } }, context.ctx),
			{
				block: true,
				reason:
					"Roast mode blocks bash commands outside its reviewed inspection policy or containing explicitly unsafe arguments.\nBlocked command: gh pr list --json number",
			},
		);
	});
});

test("active Roast mode enforces limited policy for effective bash overrides", async () => {
	await withAgentDir(async (agentDir) => {
		await writeFile(
			join(agentDir, "pi-roast-mode.json"),
			JSON.stringify({
		...
		const heredoc = `python - <<'PY'\nfrom pathlib import Path\nPath("roast-mode-write-probe.txt").write_text("unexpected write\\n", encoding="utf-8")\nPY`;
...
async function withAgentDir(run: (agentDir: string) => Promise<void>) {
	const agentDir = await mkdtemp(join(tmpdir(), "pi-roast-mode-safe-subcommands-"));
