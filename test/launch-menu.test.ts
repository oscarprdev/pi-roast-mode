...
import { readRoastModeSettings } from "../src/settings.js";
...
function launchFixtureWithSettings(readSettings: () => ReturnType<typeof readRoastModeSettings>) {
	const mock = createMockPi({
		activeTools: ["read", "write"],
		allTools: [builtinTool("read"), builtinTool("write"), extensionTool("custom")],
	});
	roastMode(mock.pi, { readSettings });
	return mock;
}

test("inactive bare /roast opens a TUI launch menu without changing Roast state", async () => {
	const mock = launchFixture();
	const tui = createTuiHarness({ width: 42, rows: 18 });
	const context = createMockContext({ mode: "tui", hasUI: true, custom: tui.custom });

	const running = mock.commands.get("roast")?.handler("", context.ctx) as Promise<unknown>;
...
test("Roast mode has no shortcut by default unless configured", async () => {
	const mock = launchFixture();
...
test("customized roast-mode shortcut from settings toggles Roast mode", async () => {
...
test("customized roast-mode shortcut from settings supports ctrl+alt+p", async () => {
	const mock = launchFixtureWithSettings(async () => ({
		kind: "loaded" as const,
...
test("the inactive launch menu opens Settings without starting Roast mode", async () => {
	const mock = launchFixture();
...
test("persisted Settings become the baseline for the next Roast workflow", async () => {
	const agentDir = await mkdtemp(join(tmpdir(), "pi-roast-mode-launch-settings-"));
	...
	try {
		...
		roastMode(mock.pi, {
			readSettings: () => readRoastModeSettings(settingsPath),
			settingsPath,
		});
...
test("the launch menu starts Roast mode only after explicit confirmation", async () => {
	const mock = launchFixture();
	const tui = createTuiHarness();
	const context = createMockContext({ mode: "tui", hasUI: true, custom: tui.custom });
...
test("launch tool choices remain draft-only until Done starts Roast mode", async () => {
	const mock = launchFixture();
	const tui = createTuiHarness();
	const context = createMockContext({ mode: "tui", hasUI: true, custom: tui.custom });
	...
	assert.deepEqual(mock.rawPi.getActiveTools(), ["read", "custom", ...REQUIRED_ROAST_TOOLS]);
	assert.equal(mock.sentUserMessages.length, 0);
});
...
test("inactive bare /roast adapts the launch menu to RPC", async () => {
	const mock = launchFixture();
	const rpc = createRpcHarness([{ kind: "select", response: "Start Roast mode" }]);
	const context = createMockContext({
		mode: "rpc",
		hasUI: true,
	...
	assert.equal(
		rpc.dialogs[0]?.title,
		"Roast mode\nStatus: Off — normal tools are active.\nWhen started: read, roast_mode_question, roast_mode_complete",
	);
...
test("RPC stages tool changes until the explicit start action", async () => {
	...
	const rpc = createRpcHarness([
		...
		{ kind: "select", response: "Done — start Roast mode" },
	]);
	const context = createMockContext({
		mode: "rpc",
		hasUI: true,
...
test("/roast tools reuses the pre-start draft and cancellation has no side effects", async () => {
	for (const ending of ["cancel", "done"] as const) {
		const mock = launchFixture();
		const tui = createTuiHarness();
		const context = createMockContext({ mode: "tui", hasUI: true, custom: tui.custom });
		const running = mock.commands.get("roast")?.handler("tools", context.ctx) as Promise<unknown>;
		await waitForOpenCount(tui, 1, running);
		assert.match(tui.render().join("\n"), /Choose Roast-mode tools/);
		assert.deepEqual(mock.rawPi.getActiveTools(), ["read", "write"]);
		assert.equal(mock.entries.length, 0);
		...
		await settleWithin(running, `${ending} /roast tools completion`);

		assert.deepEqual(
			mock.rawPi.getActiveTools(),
			ending === "done" ? ["read", ...REQUIRED_ROAST_TOOLS] : ["read", "write"],
		);
		assert.equal(
			context.statuses.get("roast-mode"),
			ending === "done" ? "roast active" : undefined,
		);
		assert.equal(mock.entries.length > 0, ending === "done");
	}
});

test("/roast tools compatibility shortcut stages directly in RPC", async () => {
	const mock = launchFixture();
	const rpc = createRpcHarness([{ kind: "select", response: "Done — start Roast mode" }]);
	const context = createMockContext({
		mode: "rpc",
		hasUI: true,
		select: rpc.ui.select,
	...
	rpc.assertConsumed();
	assert.match(rpc.dialogs[0]?.title ?? "", /Choose Roast-mode tools/);
	assert.deepEqual(mock.rawPi.getActiveTools(), ["read", ...REQUIRED_ROAST_TOOLS]);
});

test("active Roast mode locks Settings and /roast tools", async () => {
	const mock = launchFixture();
	const context = createMockContext({
		mode: "tui",
		hasUI: true,
		select: async (_title: string, options: string[]) => {
			assert.equal(options.includes("Configure Roast-mode tools"), false);
...
test("/roast tools rejects non-interactive modes before changing state", async () => {
...
test("/roast start is deterministic and bare /roast rejects non-interactive modes", async () => {
	for (const mode of ["print", "json"] as const) {
		const rejected = launchFixture();
		const rejectedContext = createMockContext({ mode, hasUI: false });
		await assert.rejects(
			rejected.commands.get("roast")?.handler("", rejectedContext.ctx) as Promise<unknown>,
		...
		await started.commands.get("roast")?.handler("start", startedContext.ctx);
		assert.deepEqual(started.rawPi.getActiveTools(), ["read", ...REQUIRED_ROAST_TOOLS]);
...
test("start is completed while longer start text remains an inline prompt", async () => {
	const mock = launchFixture();
	const context = createMockContext({ mode: "tui", hasUI: true });
	const completions = mock.commands.get("roast")?.getArgumentCompletions?.("") as
		| Array<{ value: string }>
		| undefined;
	assert.ok(completions?.some((item) => item.value === "start"));

	await mock.commands.get("roast")?.handler("start a migration", context.ctx);
