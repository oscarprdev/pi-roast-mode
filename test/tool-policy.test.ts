...
import roastMode, {
	...
	isSafeCommand,
	withoutRoastModeQuestionTool,
	withRequiredRoastModeTools,
...
test("tool selection allows safe built-ins and non-built-ins only", () => {
	type RoastTool = Parameters<typeof canSelectToolInRoastMode>[0];
	assert.equal(canSelectToolInRoastMode(builtinTool("read") as RoastTool), true);
	assert.equal(canSelectToolInRoastMode(builtinTool("edit") as RoastTool), false);
	assert.equal(canSelectToolInRoastMode(extensionTool("custom") as RoastTool), true);
	assert.equal(canSelectToolInRoastMode(extensionTool("edit") as RoastTool), true);
	assert.deepEqual(withRequiredRoastModeTools(["read", "roast_mode_question", "read"]), [
		"read",
		"roast_mode_question",
		"roast_mode_complete",
	]);
	assert.deepEqual(withoutRoastModeQuestionTool(["read", "roast_mode_question"]), ["read"]);
});
...
test("configured gh validators require exact read-only paths", () => {
	...
	for (const command of [
		...
		"gh pr view $PI_ROAST_GH_ARGUMENTS",
...
test("maximal Git opt-in preserves execution and mutation guards", () => {
	...
	for (const command of [
		...
		"git rev-parse $PI_ROAST_GIT_ARGUMENTS",
...
test("tool policy classifies built-ins and extension tools consistently", () => {
	type RoastTool = Parameters<typeof classifyRoastModeTool>[0];
	assert.equal(classifyRoastModeTool(builtinTool("read") as RoastTool), "read-only");
	assert.equal(classifyRoastModeTool(builtinTool("bash") as RoastTool), "limited");
	assert.equal(classifyRoastModeTool(builtinTool("write") as RoastTool), "blocked");
	assert.equal(classifyRoastModeTool(extensionTool("custom") as RoastTool), "user-opt-in");
});
...
test("active Roast mode blocks update_plan and blocked built-ins at the tool hook", async () => {
	...
	assert.deepEqual(blockedBuiltin, {
		block: true,
		reason: "Roast mode blocks built-in tool 'danger' because its policy class is blocked.",
	});
